...
###########################################################
### Código original, antes de aplicar a matriz do NumPy ###
###########################################################
...
def bytes2matrix(text):
    """ Converts a 128-byte array into a 4x4 matrix.  """
    return [list(text[i:i+4]) for i in range(0, len(text), 4)]

def matrix2bytes(matrix):
    """ Converts a 4x4 matrix into a 128-byte array.  """
...
def pad(plaintext):
    """
    Pads the given plaintext with PKCS#7 padding to a multiple of 128 bytes.
    Note that if the plaintext size is a multiple of 128,
    a whole block will be added.
    """
    padding_len = 128 - (len(plaintext) % 128)
...
class AES:
    ...
    """
    rounds_by_key_size = {128: 10, 24: 12, 32: 14}
    ...
    def encrypt_block(self, plaintext):
        """
        Encrypts a single block of 128 byte long plaintext.
        """
    ...
    def decrypt_block(self, ciphertext):
        """
        Decrypts a single block of 128 byte long ciphertext.
        """
...
def decrypt(key, ciphertext, workload=100000):
    ...
    assert len(ciphertext) % 128 == 0, "Ciphertext must be made of full 128-byte blocks."

    assert len(ciphertext) >= 32, """
    Ciphertext must be at least 32 bytes long (128 byte salt + 128 byte block). To
